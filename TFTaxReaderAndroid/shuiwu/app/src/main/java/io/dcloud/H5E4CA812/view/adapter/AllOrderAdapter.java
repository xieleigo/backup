...
import io.dcloud.H5E4CA812.view.activity.OrderDetailActivity;
...
public class AllOrderAdapter extends RecyclerBaseAdatpter implements PaymaxCallback {
    ...
    public AllOrderAdapter(List<AllOrderBean> dataList, Activity context) {
        this.mDataList = dataList;
        this.context = context;
    }

    public void setmDataList(List<AllOrderBean> mDataList) {
...
//        if (childAdapter == null) {
        ...
        ((RecyclerViewHolder) holder).rcvChild.setAdapter(new ChildAdapter(mDataList.get(position).getOrderDetail(), position, mDataList.get(position).getOrderInvoice()));
//        } else {
//            childAdapter.setData(mDataList.get(position).getOrderDetail()); //重新设置数据
...
//        }
    ...
    public class ChildAdapter extends RecyclerView.Adapter<ChildAdapter.ChildViewHolder> {
        public List<AllOrderBean.OrderDetailBean> childList;
        public AllOrderBean.OrderInvoiceBean childListInvoiceBean;

        public int parentIndex;

        public ChildAdapter(List<AllOrderBean.OrderDetailBean> childList, int parentIndex, AllOrderBean.OrderInvoiceBean childListInvoiceBean) {
            this.childList = childList;
            this.parentIndex = parentIndex;
            this.childListInvoiceBean = childListInvoiceBean;
        }

        @Override
        public ChildViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(context).inflate(R.layout.item_all_order_child, parent, false);
            return new ChildViewHolder(view);
        }

        @Override
        public void onBindViewHolder(ChildViewHolder holder, final int position) {
            AllOrderBean.OrderDetailBean childBean = childList.get(position);
            ...
            holder.order_layout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("orderid", (int) childBean.getOrderID());
            ...
            if (position == childList.size() - 1) {
                ...
                if (bukaifapiaoType == 10) {
                    ...
                    holder.goods_billing.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            showTicketBottom((int) childBean.getOrderID());
                        }
                    });
                } else {
                    holder.goods_billing.setVisibility(View.GONE);
                }
                if (bukaifapiaoType == 30) {
                    holder.goods_journal_lookfp.setVisibility(View.VISIBLE);
                } else {
...
//                    holder.goods_pay_layout.setVisibility(View.GONE);
            ...
            holder.goods_journal_fzjhm.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(context, AllOrderCopyJHMActivity.class);
                    intent.putExtra("OrderDetailID", ((int) childBean.getOrderDetailID()) + "");
                    context.startActivity(intent);
                }
            });
            holder.goods_journal_lookfp.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(context, BillingInformationInfoImageActivity.class);
                    intent.putExtra("URL", childListInvoiceBean.getRecvPdf51Url());
                    context.startActivity(intent);
                }
            });
        }
        ...
        public void setData(List<AllOrderBean.OrderDetailBean> childList) {
            this.childList = childList;
        }
    ...
    public void showPayBottom(double OrderID) {
        AllOrderPayPhotoPopWin payPhotoPopWin = new AllOrderPayPhotoPopWin(context);
        payPhotoPopWin.showAtLocation(context.findViewById(R.id.goods_pay), Gravity.CENTER, 0, 0);
        payPhotoPopWin.setOnChangeCountListener(new AllOrderPayPhotoPopWin.OnChangeListener() {
            @Override
            public void onChange(int payType) {
                AllOrderAdapter.this.payType = payType;
                Map map = new HashMap();
                ...
                execApi(ApiType.GetOrderSecPay, map, context);
    ...
    private OnReOrderListener OrderListener;

    public void setOnReOrderListenerListener(OnReOrderListener OrderListener) {
    ...
    public void onResponse(Request req) {
        ...
        if (req.getData() != null && req.getData().getUrl().contains("/Order/SecPay")) {
            if (payType == 5) {
                ...
                PaymaxSDK.payWithAliToken(send, context, AllOrderAdapter.this);
            ...
            if (payType == 22) {
                ...
                IWXAPI api = WXAPIFactory.createWXAPI(context, wxRequestBean.getAppid(), false);
                ...
                if (api.getWXAppSupportAPI() >= Build.PAY_SUPPORTED_SDK_INT) {
                    ...
                    ToastUtil.makeText(context, "微信版本过低");
                }
            }
        }
    ...
    public void showTicketBottom(int OrderId) {
        ShoppingCartConfirmOrderTicketPhotoPopWin takePhotoPopWin = new ShoppingCartConfirmOrderTicketPhotoPopWin(context, 0, 0);
        takePhotoPopWin.showAtLocation(context.findViewById(R.id.alloder_tabLayout), Gravity.CENTER, 0, 0);
        takePhotoPopWin.setOnChangeCountListener(new ShoppingCartConfirmOrderTicketPhotoPopWin.OnChangeListener() {
            ...
            public void onChange(Boolean isOK, String type, String UserInvoiceID) {
                if (!StringUtil.isEmpty(UserInvoiceID)) {
                    ...
                    execApi(ApiType.GetOrderRebuildInvoice, map, context);
                }
            }
        });
    }
