...
class ZHFAddTitleAddressView: UIView {
  ...
  var titleScrollView :UIScrollView = UIScrollView()
    ...
    var areaTreePath :String? // 当前点击对应的path
    
    //初始化这个地址视图
    func initAddressView() -> UIView {
        //初始化本地数据（如果是网络请求请注释掉-----
        let imagePath: String = Bundle.main.path(forResource: "lxarea", ofType: "txt")!
        ...
        cancelBtn.tag = 1
        //cancelBtn.setImage(UIImage.init(named: "cancel"), for: .normal) // lx 不显示 否则页面退下但控制器还在
    ...
    func addAnimate() {
        ...
        UIView.animate(withDuration:0.2, animations: {
            ...
            self.addAddressView.frame.origin.y = 0
        }, completion: nil)
    }
    //取消按钮被点击
    ...
    @objc func tapBtnAndcancelBtnClick() {
        UIView.animate(withDuration:0.2, animations: {
            self.addAddressView.frame.origin.y = 0 // 先 退下 ScreenHeight
        }) { (_) in
//            self.isHidden = true
            ...
            for i in 0 ..< count{
                titleAddress =  "\(titleAddress) \(self.titleMarr[i])" // 此处拼接的空格 返回带有空格
                if i == count - 1 {
            ...
            print("areaTreePath = \(String(describing: self.areaTreePath))")
            self.delegate?.cancelBtnClick(titleAddress: titleAddress, titleID: titleID, areaTreePath: self.areaTreePath ?? "")
        }
    }
}
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView.tag == 0 || tableView.tag == 1 || tableView.tag == 2 {
            if tableView.tag == 0{
                ...
                self.areaTreePath = provinceModel.areaTreePath
            }
            else if tableView.tag == 1 {
                let cityModel: CityModel = self.cityMarr[indexPath.row] as! CityModel
                ...
                self.areaTreePath = cityModel.areaTreePath
            }
            else if tableView.tag == 2 {
                let countyModel: CountyModel = self.countyMarr[indexPath.row] as! CountyModel
                ...
                self.areaTreePath = countyModel.areaTreePath
            }
        }
        else if tableView.tag == 3 {
            let townModel: TownModel = self.townMarr[indexPath.row] as! TownModel
...
extension ZHFAddTitleAddressView {
    func getAddressMessageData(addressID: NSInteger, provinceIdOrCityId: NSInteger){
        print("provinceIdOrCityId = \(provinceIdOrCityId)")
        switch addressID {
        case 1:
            self.case1()
        case 2:
    ...
    func case1() {
        ...
        if resultArr.count > 0 {
            for i in 0 ..< resultArr.count{
                ...
                if (dic["parentid"] as! String == "4000"){
                    j = j + 1
                    let dic1: [String : Any] = [
                        //记得把字符串类型转换成Int类型，否则用ObjectMapper转模型是id会出错
                        "areaTreePath":(dic["treePath"] as! NSString),
    ...
    func case4(selectedID: NSInteger) {
        ...
        #warning("LXTODO")
        self.townMarr.removeAllObjects() // 自加 此种方式可解决一时问题 但不是根本解决
