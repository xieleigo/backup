...
class JJDropDownMenu: UIView {
    ...
    /// 菜单栏标题选择颜色
    private var selectTitleColor: UIColor = TRThemeColor
    /** cell高度 */
    ...
    fileprivate var jiantouImageView = UIImageView()
...
extension JJDropDownMenu {
    ...
    fileprivate func addMenuButton() {
        ...
        for i in 0..<titleArray.count {
            ...
            selectBut.backgroundColor = LXTableViewBackgroundColor
            selectBut.titleEdgeInsets = UIEdgeInsets(top: 0, left: 14, bottom: 0, right: 0)
            selectBut.contentHorizontalAlignment = .left
            selectBut.frame = CGRect(x: CGFloat(i) * butW, y: 0, width: butW - self.menuHeight, height: self.menuHeight)
...
//            selectBut.setImage(UIImage(named: "shangjiantou"), for: .selected)
            ...
            jiantouImageView = UIImageView.init(frame: CGRect(x: selectBut.frame.origin.x + selectBut.bounds.size.width, y: 0, width: self.menuHeight, height: self.menuHeight))
            jiantouImageView.backgroundColor = LXTableViewBackgroundColor
            jiantouImageView.image = UIImage(named: "downjiantou")
            jiantouImageView.contentMode = .scaleAspectFit
            
            addSubview(jiantouImageView)
        }
    }
        ...
        if titleButton.isSelected {
            ...
            animateForBackgroundView(show: true, complete: {
                animateTableView(show: true, complete: {
                    isViewOpen = true
                    jiantouImageView.image = UIImage(named: "shangjiantou")
                })
            })
        }else {
           // 收起列表
