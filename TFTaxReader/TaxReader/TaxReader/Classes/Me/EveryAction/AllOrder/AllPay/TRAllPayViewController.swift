...
class TRAllPayViewController: UIViewController {
//    // bendi
...
extension TRAllPayViewController {
    func NetworkOrderFind(more: Bool) {
        ...
        page = (more ? ( page + 1) : 1)
        print("page = \(page)")
        networkViewModel.refreshDataSource_OrderFind(OrderStatus: "0", PageIndex: "\(page)", PageSize: "5")
    }
    
    /*
    func networkAllOrder() {
        let path = Bundle.main.path(forResource: "hqorder", ofType: "json")
        ...
        do {
            ...
            if let mappedObject = JSONDeserializer<TROrderModel>.deserializeFrom(json: json.description) {
                self.orderModel = mappedObject
                self.dataArr = self.orderModel?.data
            }
...
extension TRAllPayViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        footerView.trPayButton.tag = section
...
extension TRAllPayViewController: TROrderTableViewCellDelegate{
...
extension TRAllPayViewController: TROrderFooterViewDelegate{
    ...
    func viewInvoiceButtonDidSelected(button: UIButton, view: TROrderFooterView, model: TROrderFindDataModel?) {
        ...
        nextVc.presentBackCommitBlock = {[unowned self](userInvoiceID, selectedInvoiceType) in
            print("userInvoiceID = \(userInvoiceID) selectedInvoiceType = \(selectedInvoiceType)")
