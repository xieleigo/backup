//
//  TRAreaInlandViewController.swift
//  TaxReader
//
...
class TRAreaInlandViewController: UIViewController {

    public var backLocationStringController: ((String,String,String,String) -> Void)?
    ///title选中颜色
    public var selectColor: UIColor

    public lazy var containV: TRInlandAddressPickView = {
        let view = TRInlandAddressPickView(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: LXScreenHeight),selectColor: selectColor)
...
// MARK: - 转场动画delegate
extension TRAreaInlandViewController:UIViewControllerTransitioningDelegate {
    /// 推入动画
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = TRInlandAddressPickerPresentAnimated(type: .present)
        return animated
    }
    /// 推出动画
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = TRInlandAddressPickerPresentAnimated(type: .dismiss)
        return animated
    }
}
